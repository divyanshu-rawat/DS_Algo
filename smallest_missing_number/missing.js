...
// We can also use hashing. We can build a hash table of all positive elements in the given array. Once the hash table is built. We can look in the hash table for all positive integers, starting from 1. As soon as we find a number which is not there in hash table, we return it.
...
function firstMissingPositive(A){
    ...
    for(var x = 0; x < a.length; x++){
        ...
        if(Math.abs(a[x]) - 1 < (a.length)){

        	if(Math.abs(a[Math.abs(a[x]) - 1]) > 0) {

        		a[Math.abs(a[x]) - 1] = -a[Math.abs(a[x]) - 1]
        	}
        }
    }
